    ...
    <div>
      ...
      <Container>
        ...
        <p className={s.description}>
          Компания Auto-Repair-Shop предоставляет высококачественные услуги по починке автомобилей.
          Мы работаем ежедневно с 9:00 до 18:00. Наши специалисты обладают многолетним опытом и готовы
          помочь вам в любое время. Наш автосервис находится по адресу: ул. Примерная, д. 10, г. Москва.
        </p>
      </Container>
        ...
        <div className={s.cardContainer}>
          {serviceStore.services.map((service) => (
            <Card key={service.id} className={s.card}>
              <Card.Img variant="top" src={service.image} alt={service.name} className={s.cardImg} />
              <Card.Body className={s.cardBody}>
                <Card.Title className={s.cardTitle}>{service.name}</Card.Title>
                <Card.Text className={s.cardText}>
                  <strong>Цена:</strong> {service.cost}
                  <br />
                  <strong>Описание:</strong> {service.descriptions}
                </Card.Text>
                <div className={s.buttonContainer}>
                  <Button variant="success" onClick={() => handleOrderClick(service.id)}>
                    Order
                  </Button>
                  {orderSuccess[service.id] && (
                    <div className={`${s.alertSuccess} ${orderSuccess[service.id] ? s.show : ''}`}>
                      Успешно!
                    </div>
                  )}
                </div>
